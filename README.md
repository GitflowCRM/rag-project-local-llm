...
**Key Features:**
- 🚀 **Smart LLM Model Selection**: Automatically chooses the best model for each query type
- 📊 **PostHog Events Integration**: Process and analyze user behavior data
- 🧠 **LLM-Powered Summaries**: Generate intelligent user behavior summaries
- ⚡ **Queue-Based Processing**: Efficient batch processing with BullMQ
- 🎯 **Optimized Context**: Smart metadata extraction for better RAG performance
...
### 7.2 PostHog Events
- `POST /queue/posthog-events/find-users` — Trigger PostHog events ingestion
- `POST /llm/query` — Query user behavior data with smart model selection
- `GET /events/posthog/ingested-count` — Count ingested PostHog users
- `GET /events/posthog/uningested-count` — Count uningested PostHog users
...
### 10.2 Data Flow
1. **Events** → PostgreSQL
2. **Queue Processing** → LLM Summary Generation → Embeddings → Qdrant
3. **Query** → Embedding → Semantic Search → LLM Answer Generation
